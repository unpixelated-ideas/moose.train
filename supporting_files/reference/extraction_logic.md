...
## Prefer Static GTFS Over PDF Timetables

Railroad schedule data should be built from published static GTFS feeds when available, not from printed PDF timetables.

Required behavior:

- Configure each GTFS railroad in `supporting_files/extraction/gtfs_importer.py`.
- Download and unzip each feed into `supporting_files/source_gtfs/{feed_id}/`.
- Treat GTFS `.txt` files as CSV files.
- Build normalized records by joining `agency.txt`, `routes.txt`, `trips.txt`, `stop_times.txt`, `stops.txt`, `calendar.txt`, and `calendar_dates.txt`.
- Support optional `shapes.txt`, `transfers.txt`, and `feed_info.txt` when present.
- Namespace every source GTFS identifier with the internal feed ID. Do not assume `route_id`, `trip_id`, `stop_id`, `service_id`, `agency_id`, or `shape_id` is globally unique.
- Derive scheduled journey rows from the normalized GTFS model, not from railroad-specific parser output.
- Preserve source GTFS stop records separately from canonical Moose.train station IDs. Do not fuzzy-merge stations.
- Apply route inclusion/exclusion in feed configuration. Do not hardcode route selection inside the parser.
- Include Metro-North GTFS route IDs `3` New Haven, `4` New Canaan, `5` Danbury, and `6` Waterbury. Ignore route IDs `1` Hudson and `2` Harlem for now.
- Import the complete CTrail Hartford Line GTFS feed.
- Import only Shore Line East service from the Amtrak-hosted GTFS feed by selecting the Shore Line East agency/routes in configuration.
- Import Amtrak from the same Amtrak-hosted physical source as Shore Line East, but as a separate Moose.train service product.
- For Amtrak, include agency `51`, GTFS rail route type `2`, and only trips serving at least two configured in-scope stations.
- Emit only in-scope Amtrak stops to the user-facing schedule CSV so unsupported destinations such as Boston, Philadelphia, and Washington do not become selectable.
- Preserve GTFS after-midnight times such as `24:05` in the CSV so same-trip stop sequences remain monotonic; the prototype display wraps those times back to local clock time.
- Combine `calendar.txt` and `calendar_dates.txt`; use compact `service_days` patterns only when they exactly match the active service dates, otherwise emit explicit `service_dates`.
- Preserve GTFS traceability in `raw_notes` with `gtfs_trip_id` and `gtfs_service_id`.
- Write feed-scoped normalized CSV snapshots under `supporting_files/extraction/normalized_gtfs/{feed_id}/`.
- Record import metadata, counts, warnings, and failures in `supporting_files/extraction/gtfs_import_metadata.csv`.
- Replace one feed at a time so a failed refresh cannot corrupt another feed's existing normalized data.

Current safeguard:

- `extract_timetables.py` skips Metro-North, Hartford Line, and Shore Line East PDF parsing and emits GTFS-backed source CSV snapshots for those operators plus Amtrak.
- `gtfs_importer.py` uses one generic import pipeline for Metro-North, Hartford Line, Shore Line East, and Amtrak.

## Legacy Metro-North PDF Parsing Notes
