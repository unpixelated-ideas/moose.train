...
def main() -> None:
    ...
    rows = apply_mnr_new_haven_overlay_fixture(rows)
    rows = apply_known_service_exceptions(rows)
    rows.sort(key=lambda r: (r["agency"], r["service_name"], r["source_pdf"], int(r["source_page"]), natural_train_key(r["train_number"]), int(r["station_sequence"])))
    source_counts = {}
    for row in rows:
